...
七节点微服务一键部署（对标 KBE machine/baseappmgr/cellappmgr + 现代容器编排）：

| 服务 | 端口 | 职责 |
|---|---|---|
| Gateway | 31300 | 客户端接入网关，直连 Login/Game/Center/App |
...
| App | 31308 / 31309(HTTP) | 应用节点（游戏消息 80000-89999 + HTTP REST 应用面） |
...
- 镜像单文件（7 节点产物在 `/app/<Node>/`，compose 用 `working_dir` + `command` 指定启动）。
