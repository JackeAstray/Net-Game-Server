...
namespace App
{
    ...
    public static class AppServerApp
    {
        ...
        public static async Task StartHttpAsync(string[] args)
        {
            ...
            // 观察启动/运行期异常（端口占用、监听失败等）：RunAsync 的 Task 若不观察，Kestrel 启动失败会静默吞掉
            _ = webApiRunTask.ContinueWith(t =>
            {
                if (t.IsFaulted && t.Exception != null)
                {
                    Log.Error(t.Exception, $"App HTTP API 异常退出（监听 {bindAddress}:{httpPort}）");
                }
