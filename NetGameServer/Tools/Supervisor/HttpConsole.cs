...
using static Supervisor.Program;

namespace Supervisor;

/// <summary>
/// Supervisor 本机 Web 控制台（与 Machine.HttpConsole 对称）：
/// 零依赖 HttpListener，默认仅监听 localhost，提供：
///   GET  /            —— 控制页（纯 JS，5s 轮询 /status）
///   GET  /status      —— 托管进程状态 JSON
///   POST /control     —— 控制指令 {action: start|stop|restart, instance: "进程名"}
/// </summary>
internal static class HttpConsole
{
        ...
        CancellationToken stoppingToken)
    {
        ...
        {
            Console.WriteLine($"[Supervisor] HTTP 控制台启动失败（端口 {port}）: {ex.Message}");
            return;
        }
        Console.WriteLine($"[Supervisor] HTTP 控制台: http://localhost:{port}/");
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                _ = Task.Run(() => HandleAsync(ctx, managed, config, token));
            }
        }
        finally
        {
        ...
        string? token)
    {
        try
        {
            ...
            {
                ctx.Response.ContentType = "application/json; charset=utf-8";
                await WriteAsync(ctx, BuildStatusJson(managed));
            }
            else if (ctx.Request.HttpMethod == "POST" && path == "/control")
            {
                await HandleControlAsync(ctx, managed, config);
            }
            else
            {
        ...
        {
            Console.WriteLine($"[Supervisor] HTTP 请求处理异常: {ex.Message}");
    ...
    private static string BuildStatusJson(List<ManagedProcess> managed)
    {
    ...
    private static async Task HandleControlAsync(
        HttpListenerContext ctx, List<ManagedProcess> managed, SupervisorConfig config)
    {
        ctx.Response.ContentType = "application/json; charset=utf-8";
        ...
        var target = managed.FirstOrDefault(m => m.Config.Name.Equals(req.Instance, StringComparison.OrdinalIgnoreCase));
        if (target == null)
        {
            ctx.Response.StatusCode = (int)HttpStatusCode.NotFound;
            await WriteAsync(ctx, "{\"ok\":false,\"error\":\"进程不存在\"}");
            return;
        }
    ...
    private static bool IsRunning(ManagedProcess m)
    {
        try
        {
            var p = m.Process;
    ...
    private static void KillQuiet(ManagedProcess m)
    {
        try
        {
            var p = m.Process;
    ...
    /// <summary>控制页：纯 JS 轮询 /status，展示托管进程状态并提供控制按钮（textContent 防 XSS）。</summary>
    private const string DashboardHtml = """
<!DOCTYPE html>
<html lang="zh">
<head><meta charset="utf-8"><title>Net-Game-Server Supervisor 控制台</title>
<style>
...
<body>
<h1>Net-Game-Server Supervisor 控制台 <span id="ts"></span></h1>
<div class="card"><h2>托管进程 <button onclick="refresh()">刷新</button></h2>
<table><thead><tr><th>进程</th><th>可执行文件</th><th>PID</th><th>状态</th><th>启动/重启</th><th>日志</th><th>操作</th></tr></thead><tbody id="rows"></tbody></table></div>
<script>
...
async function refresh(){
  try{
    ...
    for(const m of st.instances){
      ...
      const cells = [m.instanceId, m.file ?? '-', m.pid ?? '-', m.startCount + ' / ' + m.restartCount, m.logFile ?? '-'];
