...
| `DB/Routing/RequestContextSession.cs` | DB 链路 `RequestId` 关联上下文 |
| `Framework/Framework.Entity/EntityPersistenceService.cs` | 实体持久化（按 EntityType 分目录文件） |

## 注意事项

- **DB 链路格式**：`[MsgId(4)][RequestId(8)][Payload]`（区别于 Gateway 链路的 `[ClientSessionId(8)][...]`），
  见 [Protocol.md](Protocol.md)。业务节点走 `Network.Routing.PacketBuilder.BuildDbRequestPacket`。
...
- **RequestId 0**：无需响应匹配时用 0（fire-and-forget），如纯写入场景。
...
| DB 链路格式错误 | 业务节点用了 `[MsgId][Payload]` 简化格式 | 必须用 `BuildDbRequestPacket` |
