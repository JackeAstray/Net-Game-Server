...
> 业务数据默认走 **EF Core + MySQL**（`DefaultDbContext` + Migrations，SchemaDoctor 自动建表/补列），
> 实体快照持久化由 `Framework.Persistence` 可插拔存储（File/MySQL/PostgreSQL/Redis）承担（Battle 使用）。
>
...
| `DB/Schema/SchemaMigrator.cs` | 结构迁移 |
| `Shared/Data/UidCounter.cs` | UID 发号段计数（原子领取） |
...
- **持久化（实体快照）**：`Framework.Persistence` 可插拔存储（File / MySQL / PostgreSQL / Redis），
  Battle 节点实体快照按 `EntityPersistence:Shards` 分片路由，单条加载 O(1) 避免全量目录扫描。
