...
namespace NetworkVerify;

/// <summary>
/// 传输层 + Battle 进程内集成验证入口。
/// 覆盖：
///   1. 传输层并发回显压测（写队列下单包原子性 + 按客户端顺序）
///   2. Battle 集成：完整协议链路（认证 → 加入房间 → 全量快照 → EntitySync → ScriptAction 伤害/击杀 → NPC 巡逻 Witness）
///   3. 断线重连（挂起 → 恢复 → 超时离场）
///   4. 静态分片（Gateway 多 Battle 节点 + 按玩家绑定路由）
///   5. 实体在线迁移（91005 协议 + Gateway 玩家 Battle 节点绑定切换）
///   6. Battle 集成压测（并发玩家加入 + 高频 EntitySync 广播 + tick 排空）
/// </summary>
