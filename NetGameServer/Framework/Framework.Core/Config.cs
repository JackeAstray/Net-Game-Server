...
/// 读取优先级（低 → 高）：appsettings.json &lt; appsettings.Local.json &lt; 无前缀环境变量 &lt; NG_ 环境变量。
...
public static class Config
{
    ...
    private static readonly object rootGate = new();

    private static IConfigurationRoot Root
    {
        get
        {
            var current = Volatile.Read(ref root);
            if (current != null) return current;
            lock (rootGate)
            {
                return root ??= new ConfigurationBuilder()
